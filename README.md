...
ORCA DB PANEL is a branded derivative distribution of [pgAdmin 4](https://github.com/pgadmin-org/pgadmin4), an open-source web administration and development platform for PostgreSQL. This repository contains the complete source tree and Docker build definition for the ORCA-branded distribution.

The application branding is configured centrally in `web/branding.py`. The implementation remains a derivative work of pgAdmin 4; upstream copyright, licence, disclaimer, and third-party notices must be preserved when distributing this project. ORCA DB PANEL is not affiliated with or endorsed by the pgAdmin Development Team or the PostgreSQL Community Association of Canada.

## Building the ORCA DB PANEL image

Build the container from the repository root:

```bash
docker build -t orca-db-panel:local .
```

Run it using the same environment and volume conventions documented for the upstream container, replacing the image name with `orca-db-panel:local`.

In the following documentation and examples, *$PGADMIN4_SRC/* is used to denote
the top-level directory of a copy of this source tree, either from a tarball or
a git checkout.

## Architecture

ORCA DB PANEL is written as a web application with Python (Flask) on the server side
and ReactJS, HTML5 with CSS for the client side processing and UI.

Although developed using web technologies, ORCA DB PANEL can be deployed either on
...
ORCA DB PANEL is dependent on a number of third-party JavaScript libraries. These,
